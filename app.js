/* ============================================
   Dompet — Money Tracker  |  js/app.js
...
function initSpinner() {
  ...
  $('txSpinnerNum').addEventListener('blur', () => {
    $('txSpinnerNum').value = txSpinnerValue > 0 ? txSpinnerValue.toLocaleString('id-ID') : '';
  });
  $('txSpinnerNum').addEventListener('focus', () => {
    $('txSpinnerNum').value = txSpinnerValue > 0 ? String(txSpinnerValue) : '';
  });

  // Budget amount input
  $('spinnerNum').addEventListener('input', e => {
    spinnerValue = parseRpInput(e.target.value);
    $('budgetLimit').value = spinnerValue;
  });
  $('spinnerNum').addEventListener('blur', () => {
    $('spinnerNum').value = spinnerValue > 0 ? spinnerValue.toLocaleString('id-ID') : '';
  });
  $('spinnerNum').addEventListener('focus', () => {
    $('spinnerNum').value = spinnerValue > 0 ? String(spinnerValue) : '';
  });
}
...
function addBudget() {
  ...
  if (existing >= 0) {
    ...
    showToast('Anggaran dibuat!', 'success');
  }
  saveBudgets();
  $('budgetCat').value = '';
  resetSpinner();
...
window.deleteBudget = function(id) {
  ...
  saveBudgets();
  renderAll();
  showToast('Anggaran dihapus', 'error');
};

function renderBudgetCards() {
  const txs = activeTxs().filter(t=>t.type==='expense');
  if (!budgets.length) {
    $('budgetCards').innerHTML = '<div class="empty-hint" style="padding:32px 0">Belum ada anggaran dibuat</div>';
    return;
  }

  $('budgetCards').innerHTML = budgets.map(b => {
    const spent    = txs.filter(t=>t.category===b.category).reduce((s,t)=>s+t.amount,0);
    const sisa     = Math.max(b.limit - spent, 0);
    const pct      = Math.min(Math.round((spent/b.limit)*100), 100);
    const color    = pct >= 90 ? 'var(--red)' : pct >= 70 ? 'var(--yellow)' : 'var(--green)';
    const cat      = getCat(b.category);
    return `<div class="budget-card">
      <div class="bc-top">
        <div class="bc-cat">
          <div class="bc-icon" style="background:${cat.color}18">${cat.icon}</div>
          <div>
            <div class="bc-name">${cat.label}</div>
            <div class="bc-limit">Batas: ${fmt(b.limit)}</div>
          </div>
        </div>
        <div class="bc-actions">
          <button class="bc-del" onclick="deleteBudget(${b.id})">Hapus</button>
        </div>
      </div>
      <div class="bc-stats">
        <div>
          <div class="bc-stat-label">Terpakai</div>
          <div class="bc-stat-val" style="color:${color}">${fmt(spent)}</div>
        </div>
        <div>
          <div class="bc-stat-label">Sisa</div>
          <div class="bc-stat-val" style="color:var(--green)">${fmt(sisa)}</div>
        </div>
        <div>
          <div class="bc-stat-label">Terpakai</div>
          <div class="bc-stat-val" style="color:${color}">${pct}%</div>
        </div>
      </div>
      <div class="bc-track"><div class="bc-fill" style="width:${pct}%;background:${color}"></div></div>
      <div class="bc-pct-row"><span>${pct}% terpakai</span><span>${100-pct}% tersisa</span></div>
    </div>`;
  }).join('');
}
...
function checkBudgetAlert(category, amount) {
  ...
  if (!budget) return;
  const txs  = activeTxs().filter(t=>t.type==='expense' && t.category===category);
  const spent = txs.reduce((s,t)=>s+t.amount,0);
  const pct   = (spent / budget.limit) * 100;
  if (pct >= 100) showToast(`⚠️ Anggaran ${getCat(category).label} sudah habis!`, 'warn');
  else if (pct >= 80) showToast(`⚠️ ${getCat(category).label} sudah ${Math.round(pct)}% dari anggaran`, 'warn');
}
